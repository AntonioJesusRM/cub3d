...
int	texture_data(char *line, t_dates **dates, int *ctrl_data, int *ini_map)
{
	...
	if (ft_strncmp(line, "NO", 2) == 0)
		ctrl = set_texture(dates, &ctrl_data, line, 0);
	else if (ft_strncmp(line, "SO", 2) == 0)
		ctrl = set_texture(dates, &ctrl_data, line, 1);
	else if (ft_strncmp(line, "WE", 2) == 0)
		ctrl = set_texture(dates, &ctrl_data, line, 2);
	else if (ft_strncmp(line, "EA", 2) == 0)
		ctrl = set_texture(dates, &ctrl_data, line, 3);
	else if (ft_strncmp(line, "F", 1) == 0)
		ctrl = set_texture(dates, &ctrl_data, line, 4);
	else if (ft_strncmp(line, "C", 1) == 0)
		ctrl = set_texture(dates, &ctrl_data, line, 5);
	else
...
int	set_texture(t_dates **dates, int **ctrl_data, char *line, int i)
{
	...
	if ((*ctrl_data)[i] == 0)
	{
		...
		if (i == 0)
			ctrl = put_texture(&(*dates)->no, line);
		else if (i == 1)
			ctrl = put_texture(&(*dates)->so, line);
		else if (i == 2)
			ctrl = put_texture(&(*dates)->we, line);
		else if (i == 3)
			ctrl = put_texture(&(*dates)->ea, line);
		if (i == 4)
			ctrl = put_color(&(*dates)->c, line);
		if (i == 5)
			ctrl = put_color(&(*dates)->f, line);
	}
	else
	{
		ctrl = 0;
